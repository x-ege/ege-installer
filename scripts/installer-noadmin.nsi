; EGE Installer NSIS Script (No-Admin 版本)
; 使用 NSIS 打包 HTA 安装界面
; 静默解压并启动 HTA 界面
; 此版本不请求管理员权限，启动时无需 UAC 确认

;--------------------------------
; 基本设置

Name "EGE 图形库安装程序"
OutFile "..\dist\ege-installer-${VERSION}-noadmin.exe"
Icon "..\assets\ege-icon.ico"  ; 设置安装程序图标
Unicode True
RequestExecutionLevel user  ; 不请求管理员权限，以当前用户权限启动
...
VIAddVersionKey "ProductName" "EGE 图形库安装程序 (免管理员版)"
VIAddVersionKey "CompanyName" "EGE Project"
VIAddVersionKey "FileDescription" "EGE (Easy Graphics Engine) 安装程序 - 免管理员版"
...
Section "Main"
    ...
    ; HTA 通过 net session 自动检测管理员权限，无需标志文件
