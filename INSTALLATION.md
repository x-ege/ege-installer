...
1. 打开 Red Panda C++
2. 点击 **文件 → 新建 → 项目...**
3. 选择 **Console Application（控制台应用）**
4. 输入项目名称和保存路径
5. 点击 **确定**
...
- A: 确保链接器参数配置正确，参数顺序也很重要（`-lgraphics` 要放在最前面）
...
CLion 是 JetBrains 出品的跨平台 C/C++ IDE，支持 CMake 构建系统。CLion 本身不包含编译器，需要配置工具链。

#### 🚀 创建 EGE 项目

1. **安装 MinGW 编译器**
   - 可以使用 Red Panda C++ 内置的 MinGW
   - 或单独安装 MinGW-w64 / MSYS2
...
4. **配置 CMakeLists.txt**
   ...
   project(my_ege_project)
   
   set(CMAKE_CXX_STANDARD 17)
   
   add_executable(my_ege_project main.cpp)
   
   # 链接 EGE 和所需的 Windows 库
   target_link_libraries(my_ege_project
       graphics
       gdiplus
       gdi32
       imm32
   ...
   # Windows 子系统和静态链接
   if(WIN32)
       target_link_options(my_ege_project PRIVATE -mwindows -static)
   endif()
   ```
...
- A: 确保通过安装器安装到了正确的 MinGW，并在 CLion 中配置了该工具链

**Q: 链接错误？**

- A: 检查 `CMakeLists.txt` 中的链接库顺序和参数
